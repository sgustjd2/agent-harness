# Experiment B — Skill script path resolution (Claude Code)
...
| 2 | Host-provided path variable | `${CLAUDE_SKILL_DIR}` is documented. If it works, record that it works AND that it still cannot be used in the canonical layer (FR-027-B rule 3) -- adapters only. |
